...
function CardForm({
  ...
  const [kind, setKind] = useState<DeskTaskKind>('task');
  const [title, setTitle] = useState('');
  const [body, setBody] = useState('');
  const [priority, setPriority] = useState<DeskTaskPriority>('p2');
  const [folder, setFolder] = useState(workspaceFolders[0]?.path ?? recentFolders[0] ?? '');
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        if (!canAdd) return;
        desk.addCard({ kind, title: title.trim(), body: body.trim(), priority, folder });
        onDone();
      }}
...
function Card({
  ...
  return (
    <div
      className={`flex flex-col border-2 bg-bg shadow-pixel ${
        yours ? 'border-status-permission' : 'border-border'
      }`}
    ...
    >
      ...
      {isOpen && (
        <CardDetail task={task} desk={desk} labelOf={labelOf} canStartAgents={canStartAgents} />
      )}
    </div>
  );
}
...
export function TaskDesk({
  ...
  const sections = deskSections(desk.tasks);
  const waiting = sections.needsYou.length;
  ...
  const list = (title: string, tasks: DeskTask[], empty: string) => (
    <div className="flex flex-col gap-6">
      ...
      {tasks.length === 0 && <span className="text-2xs text-text-muted">{empty}</span>}
      {tasks.map((task) => (
        <Card
          key={task.id}
          task={task}
          isOpen={openId === task.id}
          onOpen={() => setOpenId((id) => (id === task.id ? null : task.id))}
          desk={desk}
          labelOf={labelOf}
