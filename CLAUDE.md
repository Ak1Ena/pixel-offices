...
- **Open file** (toolbar, and inside the viewer; standalone + private link): `OpenFileDialog.tsx` browses this machine with `listFolder {files: true}` (sub-folders plus the files `isViewableName` accepts, `FolderFile`), a pasted path, or recent files (localStorage `OPEN_FILE_RECENT_KEY`). A picked file opens IN PLACE through `openFileInViewer` (an existing file pin for the path is reused, else one is pinned) — so edits save to the real file; Upload opens the office's copy under `~/.pixel-agents/files`.
