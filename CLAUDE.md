...
server/                              Lifecycle runtime + Fastify HTTP/WS server
  src/
    ...
    contextClear.ts                  /clear from the office or on an agent's own request (clearPolicy); clearCli.ts = `pixel-office clear`
    officeDocs.ts                    Word/PowerPoint/Excel: numbered places + in-place XML edits (jszip + xmldom); docCli.ts = `pixel-office doc`
    docEdits.ts                      Document writes: sha check, backup, undo, per-agent docEditMode (ask → proposal, auto, off)
...
webview-ui/                          React 19 + Canvas UI (depends only on core/)
  src/
    ...
    components/DeskSteps.tsx         Card step editor (drag/↑↓, kind cycle, ref, gate answers)
    components/DocModelViews.tsx     Word paragraphs + PowerPoint slides, pick and edit in place
    textPrefs.ts, tunables.ts        Text size/font prefs and the Advanced-settings registry (both localStorage, per viewer)
...
`HookProvider` (`core/src/provider.ts`) is the integration boundary. Implemented: Claude Code (reference, transcripts + hooks), plus hooks-only Codex CLI, Gemini CLI, Antigravity CLI (`agy`: one named `pixel-agents` spec in `~/.gemini/config/hooks.json`; PreToolUse NOT installed — any reply there is a permission decision, `{}` denies; the hook script fills event/session/cwd via `HookScriptOptions.expand` and prints `{}`; office-run agy — `pixel-office agy`, + Agent — is linked by pid: first event carries `cli_pids` (hook ancestors; `ps` / PowerShell CIM on Windows), `runtime.followLaunchedPid(pid, key, cwd)` matches the terminal's pid, sets `agent.launchKey` for the writers, bypasses Watch All and fills a missing cwd) and a Generic HTTP provider (`server/src/providers/hook/{codex,gemini,antigravity,generic}/`, routed by the `:providerId` in the hook URL, adopted as `hooksOnly` agents; see `docs/providers.md` — Codex needs `/hooks` trust, Gemini's permission prompt can't be answered from a hook). The Claude provider supports every transcript/hook format up to **Claude Code v2.1.220** (current as of 2026-07-30 — Task-era `agent_progress` records, explicit and implicit teams, background-by-default Agent spawns, sidecar-backed background agents). Newer CLI releases may add formats that need provider updates. The interface:
...
- **Who is in the office** (`pixel-office agents [--json]`, `agentsCli.ts` → Bearer `GET /api/agents?session=`, Origin refused; `officeRoster.ts`): every agent whatever CLI runs it, by office name, with provider, status, current tool, folder and lead. Claude's own agent list knows Claude sessions only, so agents are told this command (team first messages, group-chat note). The caller's `CLAUDE_CODE_SESSION_ID` / `ANTIGRAVITY_CONVERSATION_ID` marks its own entry `(you)`.
...
## Decision model (optional, Laya)

- `server/src/decisions.ts`: a client for the "System One" wire protocol (`POST <url>/v1/systemone`, `{state, questions}` → `{answers: {key: {choice|noul, confidence}}}`), shared by Laya's `laya-serve` (open weights, Apache 2.0, run locally: `pip install "laya[serve]"`, `LAYA_HOST=127.0.0.1 laya-serve`) and TypeSafe's hosted Jev. **Off unless configured**: `PIXEL_AGENTS_DECISIONS_URL` (+ `PIXEL_AGENTS_DECISIONS_KEY`) or config.json `decisions: {url, apiKey?}`, read once per runtime (`runtime.decisions`). The text sent is agent replies — point it at an endpoint the user runs.
- **The rule runs first; the model only reads what the rule can't settle; only an answer at or above its threshold is used — per Laya checkpoint (the reply's `model`) and question kind, `DECISION_THRESHOLDS`; Laya's `confidence` is not P(answer), correct answers mostly score 0.1–0.6; unknown servers use `DECISION_MIN_CONFIDENCE` (0.85); recalibrate with `scripts/laya-eval.py`; a failure, timeout (`DECISION_TIMEOUT_MS`) or unsure answer = the rule's behavior.** Never used for permissions, consent or file writes.
- Three readers: (1) **task desk** `turnEnded` — a build turn ending without `task done` is read as finished / question / blocked; question or blocked keeps the card `working` with `waitingOn` (Needs you, reply box types into the session via `sendChatMessage`; the claim stays, so the next turn end is read again; `workResumed` clears it when the agent works). (2) **relay + team call-ins** (`addressedDecisions.ts`) — a paragraph the opening-`@` rule gives to nobody but that `@`-mentions someone later is asked "is this a request to X?". (3) **hooks-off text-only turn end** (`textIdleJudge.ts`) — "finished" idles now, "continuing" waits `TEXT_IDLE_CONTINUING_DELAY_MS`; stale once new data replaced the timer.
- **Laya managed from the office** (`layaManager.ts`, Settings → Decision model; `setLayaEnabled` / `setLayaModel` / `uninstallLaya`, privileged, → `layaStatus`): off until turned on (config.json `laya: {enabled, model}`). On = first Python 3.10+ on PATH builds a venv in `~/.pixel-agents/laya/venv`, `pip install laya[serve]` (Linux: CPU torch index), weights under `laya/hf` (HF_HOME), then `laya-serve` on 127.0.0.1 with a random port + bearer key. `serve.json` lets a second office reuse it (same `models` only); a 30 s watchdog restarts it. Started as `venv python -m laya.serve`, probed on `/health`; stopped with `killTree` (Windows `taskkill /T` — a venv python.exe is a redirector whose CHILD is the real interpreter). Off stops it (download stays); Uninstall stops ours AND a shared one, waits for exit, then deletes the whole folder (rm retries for Windows file locks). Language: `english`, `multilingual`, or `auto` (both preloaded, no `model` in requests — Laya's router picks). A hand-set endpoint always wins (`runtime.decisions`).
- **Card routing** (`cardRouting.ts`, `suggestDeskCard` → `deskCardSuggestion` to the asker): "Suggest team, workflow and model (Laya)" in the card form asks three `choice` questions over this office's team presets, workflows and the cached model picker (ModelCatalog), each with a way out (`solo` / `freeform` / `default`). Confident picks fill the form only; the human saves. Cards carry an optional `model` (preselected in "Start an agent here").
- **Desk autopilot** (`deskAutopilot.ts`, owned by `TaskDesk`, Settings → Autopilot; `setAutopilot` privileged → `autopilotState`; config.json `autopilot: {enabled, maxAgents, idleMinutes, command}`). Each desk tick: `plan` (before the hand-out) routes a new inbox card once (`autoRouted`) — the decision model fills only the team/workflow/model the human left empty, else "one agent, usual model" — and builds a brief without `high` risk (`autopilotBuild`) unless the model confidently says the plan needs the human (`plan` question); unanswered brief questions each get a `choice` (agent / user) and, only when ALL read as the agent's to pick (`agentAnswers`), are filled with `AUTOPILOT_AGENT_CHOOSES`; `passGates`: a waiting gate step the model reads as routine is answered continue (logged as Autopilot); `check`: a result with no `--tests` or failing tests (`testsFailed`) is sent back, ≤ `AUTOPILOT_MAX_SEND_BACKS`, and a result not on its own branch gets a note; the build prompt carries `buildNote` (own branch, no push, run tests, list choices). Past the inbox autopilot only touches cards it routed (`autoRouted`). Permission prompts are NOT auto-answered; `staff` (after it) starts an agent via `OfficeSessions` for cards no free agent can take (one per waiting card, agents on their way count, ≤ `maxAgents` of its own, a failed start logs once) and closes its OWN agents idle with no claim for `idleMinutes`. Cards being read are `holds()` — the hand-out skips them. Never answers permissions, agent questions or results. Picks are logged on the card (who: Autopilot).
- **Bare-name call-ins**: `namedIn` (whole word, ≥ `ADDRESS_BARE_NAME_MIN_CHARS`) adds candidates the rule never acts on — only a confident decision-model yes addresses them.
- **Laya reads the board** (`taskDesk.ts`): at every build turn end, open steps the agent never reported are asked about (`readSteps`, one yes/no each, `step` threshold) and ticked by `stepsReadDone` (no skipping, logged as Laya); the CLI reports stay the exact path.
- **Board columns** (`deskFlow.ts`, `~/.pixel-agents/desk-flow.json`, `deskFlowLoaded` / `saveDeskFlow` / `setDeskColumn`, Full board → Columns, `DeskFlowEditor.tsx`): a column SPLITS one card state (`phase`) with a name and a description; state moves stay with `taskTransitions.ts`, and `commit()` puts a card that changed state into its new state's first column (`placeCard`). Inside a state: the human drags (`setColumn`), and Laya moves it (`placeByReply`, `column` threshold) into columns marked `laya`, judged by their descriptions — on an agent reply mid-turn (≤ one read per card per `DESK_COLUMN_READ_MS`) and after each turn end. Webview: `subColumns` splits a board column into sections.
- **Laya in the office** (`officeState.laya`, `spawnLaya`/`consultLaya`, like the greeter: drawn via `getCharacters`, never in `characters`): shown while a decision model answers (`decisionModelReady`). Every `Decider.ask` carries `about {agentId, topic}`; the runtime's wrapper (`watched`) broadcasts `layaConsult` (≤ one per agent per `LAYA_CONSULT_COOLDOWN_MS`) and the agent walks over, shows "Laya, <topic>" while talking (`LAYA_TALK_SEC`), and returns to its desk — a busy agent only shows the question.
- Laya's base checkpoints are weak zero-shot on typed decisions (its own README); measure on this office's replies before trusting it, and fine-tune if needed.

## Clear context

- **Clear** (`clearAgentContext {id, mode: clear|compact}`, privileged; chat card "Clear…"): `ContextClear` (`server/src/contextClear.ts`, owned by `AgentRuntime`) types `/clear` (or `/compact`) through `ChatSender` — so it waits for the turn to end and never lands on a permission prompt — and NOTHING after it: no hand-off, the agent starts blank. Only agents the office can type into (`agentChatSendable`).
- **The character survives the clear** because the runtime already follows `/clear` onto the new transcript (`reassignAgentToFile`, via hooks or content detection). `reassignAgentToFile` sets `agent.launchKey ??= <old sessionId>`: office-run ptys and the launcher know a session by the id it was STARTED with, so without it a cleared agent could no longer be typed into, and the launcher's poll (`adoptLaunchedSession`, which now also matches `launchKey`) re-adopted the old transcript as a duplicate. `launchKey` is persisted.
- **An agent asks for its own clear**: `pixel-office clear [--reason T]` (`clearCli.ts`) → Bearer `POST /api/agents/clear {session}` (Origin refused; 404 = not this office's session, the CLI tries the next). Per-agent `clearPolicy` (persisted, `agentPrefs`): `ask` (default — the request shows in the chat card and the prompt stack, `agentClearRequests`), `allow` (clears after the turn), `never` (refused). Team first messages mention the command.
- **Task desk cards need no hand-off**: a claim lasts one turn, and a queued `/clear` is typed only after that turn ended — the desk has already moved the card on.
...
- **City Office**: furniture `OFFICE_DESK`, `DUAL_MONITOR` (electronics, on/off), `CITY_WINDOW`, `PLANTER`, `CUBICLE_DIVIDER` plus the bundled preset `webview-ui/src/office/layout/presets/cityOffice.json`, applied from Settings → Use City Office Layout as an undoable editor edit (Undo/Reset restore the old layout). Settings → Use Original Office Layout switches back the same way, to `webview-ui/public/assets/default-layout-1.json` (imported directly — the same file the server gives new users, one copy). Floor pattern 3 (low contrast) — pattern 9 is a checkerboard.

## Agents the office runs (+ Agent in the browser)

- `startAgent` (privileged, standalone) → `OfficeSessions` (`server/src/officeSessions.ts`) spawns Claude in a node-pty the server owns — no terminal window. `planLaunch` mints the session id; the command may be an alias but must run Claude. The agent is adopted at once via `runtime.adoptLaunchedSession` (retried each second; the transcript only appears with the first prompt). `OfficeSessions.writer` is a third `TerminalWriter` for `ChatSender`. `closeAgent` kills the pty; pty exit removes the agent. Max `OFFICE_SESSION_LIMIT`. **Agents the office runs die with it — and stay gone**: `adopted()` marks them `officeRun` (persisted), `restoreExternalAgents` never restores those (an unclean stop left them saved), and on shutdown (`SIGINT`/`SIGTERM`/`SIGHUP` — closing the terminal) their transcripts go to `~/.pixel-agents/ended-sessions.json`, which the next office dismisses at startup (`dismissEndedSessions`, the user-close cooldown) — without it the external scanner re-adopted them as read-only ghosts because their transcripts were seconds old. A resumed session that writes again comes back.
...
- **Models come from the CLI's own picker, never a list in the office** (`HookProvider.modelPicker`, Claude: `{command: '/model', sessionKey: 's'}`). `OfficeSessions.pick` types the command, reads the numbered dialog off the screen (`parseScreenQuestion` → `parseModelOptions`: name column = label, `✔` = current), then presses Esc (read) or arrows + the session-only key (switch — `s`, so the user's default model for new sessions is untouched; verified live). While `picking`, the writer is not ready and the dialog is not shown to people as a question. `loadAgentModels`/`setAgentModel` (privileged, office-run agents only, refused mid-turn) → `agentModels`. Each read is cached per provider in `~/.pixel-agents/model-options.json` (`ModelCatalog`) and sent as `modelOptions` for start forms; `startAgent {model}` holds the first message off the command line, picks the model once the terminal is ready, then types it.
- **Model switch robustness**: every picker step reads a SETTLED screen (`settledQuestion`, `MODEL_PICKER_SETTLE_POLLS` equal polls) — acting on the frame the picker draws over the slash-command list moved the cursor from the wrong row. The cursor position is re-checked before `s`, and the switch is confirmed by re-opening the picker; after a switch Claude repaints and drops keys, so `openPicker` waits for a still screen and retries once with Ctrl+U (never Esc: at an idle prompt it opens the rewind menu). Local commands (`<command-name>`, `<local-command-…>` records) are NOT turns (`isLocalCommandRecord`): they write no turn_duration, and marking them busy left the agent "active" for good.
- **`/` menu** (`useSlashMenu`, `slashMenu.ts`): the list is the CLI's own — `HookProvider.listSlashCommands(cwd)`; Claude runs `claude -p --output-format stream-json --verbose --no-session-persistence` and kills it at the `system/init` record (`slash_commands`), before any model request; `HOOK_PROBE_ENV` makes our hook scripts exit silently so the probe never shows up as an agent. Cached per provider + folder (`SlashCommands`, `SLASH_COMMANDS_CACHE_MS`). `listSlashCommands` → `slashCommands` (privileged, to the asker only). Offered where the office can type into the agent.
- **Resume** (+ Agent → Resume a session): `listPastSessions {cwd}` → `pastSessions` (privileged, to the asker): the folder's transcripts newest first, titled by `agent-name` (user-set) > newest `ai-title` > first real prompt, read from head + tail only (`pastSessions.ts`). `startAgent {resume}` adds `--resume <id>` (planLaunch keeps that id); a session already open in the office is refused.
...
- **Who is free**: top-level session (no `leadAgentId`, no `spawnToolUseId`, not `hooksOnly`), `isWaiting`, no `permissionSent`, `chatSender.canSend` + `isIdle`, pick-up on, same root, on the card's `allow` list (empty = anyone in the folder), not already holding a card. **Pick-up defaults ON only for agents the office started** (`runtime.deskDefaultPickup` = `officeSessions.owns`); a session somebody started themselves is off until switched on, so the desk never types into work in progress. **Read-only sessions can never take a card**: the "who may look" list can't tick them (only untick), `setAllow` refuses a list where no agent is reachable (it would wait forever), and a card stuck on one says so (`stuckReason`) with Let any agent here take it / Start an agent here (`stuckFixes`).
...
- **Steps** (a brief's `subtasks`, `DeskSubtask`): each has a stable `id` (`s` + 6 hex, `withStepIds`), a `kind` like a workflow step (`do` default, `gate`, `show`; agents write `"[gate] …"` in brief.json), an optional `ref` (file + place). The human edits them in `DeskSteps.tsx` — drag ⠿ or ↑↓, click the kind to cycle, `@` for the ref, skip, remove, add — while the brief waits (sent with verify/do) and DURING the build (`editDeskSteps`). Mid-build, `lockedSteps` (done/skipped steps + the current one) must come first unchanged; the rest is free. The agent learns on its next `task step`/`task gate` reply (`planChanged`, once — `takePlanChange`), never by typing. Out-of-order reports skip the gap, like workflows.
- **Gates**: `pixel-office task gate <num> <step> [--ask Q]` → `POST /api/tasks/:ref/gate` marks the step `waiting` (+ `ask`), then long-polls `GET …/gate/:step` (`TASK_GATE_POLL_MS`). The human answers in the card or the prompt stack (`answerDeskGate continue|stop`, `deskGates()`); an answer given between polls is kept (`gateAnswers`); a card that stops being built settles waiters with `gone`. `show` steps use `pixel-office show`, then `task step`. Save as workflow / Load workflow convert steps ↔ workflow steps (`stepsToWorkflow`, `workflowToSteps`).
...
- **Team, workflow, files on a card** (`teamId`, `workflowId`, `attachments`; team/workflow editable until an agent looks, files until the build): a TEAM card makes the desk start the preset in the card's folder (`TaskDesk.startTeams` → `DeskTeams.start` = `runtime.crews.start`, goal = `teamGoal`: read the card, change nothing yet), store `crewId`, and hand the card ONLY to that crew's lead (`TeamRuns.leadOf`; `allow` ignored). A failed start is logged on the card once (`teamFailed`, cleared by an edit). A WORKFLOW card: `task show` (`TaskDesk.describe`) lists the workflow's steps and the look prompt says to use them as the brief's subtasks. Attachments are absolute paths that must be files (uploads go through `/api/files`); agents get paths in `show`, never content.
...
- Whiteboard **file pins** open inside the standalone office (`DocViewer.tsx`): PDF and images natively (blob URL), Word laid out like Word by `docx-preview` (`WordDocumentView.tsx`) into a frame that runs NO scripts (`sandbox="allow-same-origin"` without `allow-scripts`; altChunk HTML off; links intercepted, web links open outside), Excel from the server model (fallback `read-excel-file`) and CSV as a table (capped at `DOC_TABLE_MAX_ROWS`), text as text. Libraries are dynamic imports, loaded only when such a file is opened. The ¶ numbers are laid over the drawn Word paragraphs by text (`matchParagraphs`: body `article p` only, look-ahead so a text box can't shift the rest), so a click in the Document view picks the same ¶ an agent reads.
...
- **Office documents have numbered places** (`core/src/docModel.ts`; parsed on the server by `officeDocs.ts` — jszip + @xmldom/xmldom): Word paragraphs `¶n` (tables flattened row by row, empty paragraphs counted), PowerPoint slides + named text boxes, Excel cells with formulas. The viewer fetches `GET /api/board/files/:pinId/model` (`{sha, model}`) and shows Word as the laid-out Document (default) or the numbered Paragraphs list (used for editing), PowerPoint as slides, Excel from the model (formulas in the formula bar). Refs: `[@path paragraphs 3-4]`, `[@path slide 2 "Content 3"]` — never the text. Agents resolve them locally with `pixel-office doc read FILE --para 3-4 | --slide 2 [--shape N] | --cell RANGE` and find numbers with `doc outline`. `pixel-office show --para/--slide` travel as lines/page.
- **Editing documents** (`docEdits.ts`, `runtime.docs`): an edit names a PLACE (`DocEdit`: `para`, `insertAfter`, `shape`, `cell`) and `applyDocEdits` patches only that XML — styles, images, charts, other sheets survive; a batch is all-or-nothing and numbered against the document before the batch. Every write: sha check against the version the editor saw (409 on a change), backup to `~/.pixel-agents/backups/`, atomic replace; Undo (`undoDocEdit`) only while the file still holds what was written, only the newest edit of a file. The human saves from the viewer (`POST /api/board/files/:pinId/edits {sha, edits|text}`; text formats in `isTextEditable` only). **Agents**: `pixel-office doc edit FILE --para N --text T | --insert-after N | --slide N --shape S | --cell REF --value V | --from edits.json [--why] [--wait]` → Bearer `POST /api/docs/edits` (Origin refused). Per-agent `docEditMode` (chat card ⚙, `agentPrefs`), else the machine-global `docEditDefault` (config.json, Settings): `ask` → a document suggestion in Review changes (`Proposals.openDoc`; one hunk per edit with `where`, `applyAny` re-checks the sha and re-previews on change), `auto` → written at once with a `docEdits` notice (Undo), `off` → refused. An open viewer reloads when an edit to its file lands (`lastEditKey`), or warns when the human is mid-edit.
- **Open file** (toolbar, and inside the viewer; standalone + private link): `OpenFileDialog.tsx` browses this machine with `listFolder {files: true}` (sub-folders plus the files `isViewableName` accepts, `FolderFile`), a pasted path, or recent files (localStorage `OPEN_FILE_RECENT_KEY`). A picked file opens IN PLACE through `openFileInViewer` (an existing file pin for the path is reused, else one is pinned) — so edits save to the real file; Upload opens the office's copy under `~/.pixel-agents/files`. **Uploads never duplicate**: the same name + content returns the stored copy and its existing pin (`findStoredCopy`). **Deleting**: `removeBoardPin {deleteFile}` (privileged) deletes the file only when `isStoredUpload` — its REAL path sits directly in `~/.pixel-agents/files` with the `pin_` prefix — and no other pin uses it (`removePinAndCopy`); the user's own files are never deleted. The whiteboard's Remove on such a pin asks Delete file / Pin only; the viewer has Delete file.
- **Agent chat** (viewer header, and the pick bar): `DocChatPanel.tsx` beside the open document — a conversation with one agent about the file. The agent is chosen once per file (remembered in localStorage `DOC_CHAT_AGENTS_KEY`; default: the open chat's agent, else `rankAgents`: in the file's folder via the desk's `root`, idle first) or started new in the file's folder (`startAgent`, the question as its first message; the new agent is the one id that appears next, `newAgentId`). The thread is the agent's own chat (`chat.chats`), shown compactly; the first message names the file (`askMessage`), later ones carry only the places picked since (`followUpMessage`). Refs, never text. The agent's `doc edit`s reach the open viewer (reload / Review changes).
- **Suggestions show in the document** (office files only; text files keep `ReviewPanel`): each doc hunk carries `place` (`DocPlaceRef`: `para`, `insertAfter`, `slide`+`shape`, `cell`+`sheet`), so the viewer (`docSuggestions.ts` `placeSuggestions`, `DocSuggestions.tsx`) strikes the old text where it is and puts the new text under it with ✓ Accept / ✗ Reject: in the laid-out Word page (cards injected into the no-script frame; clicks handled by the office page), the Paragraphs list, the slide's text box, and the sheet (cell highlighted + a list). `SuggestionBar` = Accept all / Reject all / Discard / Apply N; after Apply the viewer reloads (the proposal's state is part of `lastEditKey`) and offers Undo. The "Review changes" notice opens the file for office documents.
- **Files** (toolbar Files, `FilesRail.tsx`; standalone + private link): the documents the office opened, kept in `~/.pixel-agents/files.json` (`officeFiles.ts`: paths only, sanitized, atomic, polled; ids `f` + 12 hex) — SEPARATE from the whiteboard, so opening a file never pins it (agents see only pins, via `board.md`). The viewer fetches by FILE id: `openOfficeFile {path}` → `fileOpened {fileId}` (to the requester), then `GET/POST /api/docs/files/:fileId[/model|/edits]` — the same `registerViewerFileRoutes` + `resolveViewableFile` checks as the pin routes (which stay). Whiteboard pins, "show me", chat links and Review all open through Files by path. Tabs: Recent (pinned / copy / to review / missing tags; Pin to board = `pinOfficeFile`, Remove from list), Review (open proposals), Uploads (`deleteOfficeUpload`: only `isStoredUpload` copies, their pins too), Backups. Uploads from Open file use `?pin=0` (listed, not pinned); whiteboard uploads still pin.
- **Backups** (`backups.ts`): `<ms>-<id>-<pathhash8>-<name>` in `~/.pixel-agents/backups`, grouped per file (`listBackups`), pruned after every write (`pruneBackups`: newest `BACKUPS_KEEP_PER_FILE`=5, none older than 7 days), `clearBackups {key?}`. Writes (docEdits commit/undo, proposals apply/undo) call the runtime's `fileWritten` → prune + `files.markEdited` + `filesLoaded`.
- **Suggestions survive restarts**: `Proposals` saves open entries to `~/.pixel-agents/suggestions.json` on every change (runtime only; tests pass no path); agent ids are dropped on load.
...
**Text + tunables**: the type scale is CSS variables (`--fs-*`, `--fs-read*`, `--fs-code*` × `--font-scale` × per-font `-k` factors, `index.css`); Settings → Text sets size and the message/interface fonts (`textPrefs.ts`, `useTextPrefs`, localStorage `TEXT_PREFS_KEY`; the Messenger's reading menu is the same store). Settings → Advanced exposes user-facing webview constants (`tunables.ts` registry with bounds; `tunable(key)` read at use, `useTunables()` in React; localStorage `TUNABLES_KEY`, only non-defaults stored). A constant stays the default — layout math, sprites, protocol and colors are never tunables.

**UI styling** (Soft Dollhouse, matching the 3D office): warm dark panels (`--color-bg` #17191e on `--color-bg-dark`), coral accent (`--color-accent`, text on it `text-accent-ink`), 1px `border-border`, rounded corners (`rounded-ui` 10px for controls, `rounded-panel` 14px for panels; `.pixel-panel` = panel + border + soft shadow), soft shadows (`shadow-pixel` now resolves to a blur shadow). Faces: Figtree (interface + reading, `@fontsource-variable`), Bricolage Grotesque for titles (`font-display`), JetBrains Mono for code — all bundled, no network. Settings → Text can still pick the Pixel face (FS Pixel Sans). Tokens live in `index.css` `:root`; components use the Tailwind tokens, never literals. The bottom toolbar is a centred icon dock; in 3D the office adds an "In the office" roster (`OfficeRoster`), a clock (day/night, next stand-up) and name tags. Custom ESLint rules (`eslint-rules/pixel-agents-rules.mjs`) still enforce `no-inline-colors` (hex/rgb literals only in `constants.ts` files, incl. `core/src/constants.ts`), `pixel-shadow` (use `var(--pixel-shadow)`) and `pixel-font` — all `error`-level.
...
- **Colour look**: `colorMode.ts` turns three.js colour management off and renders in linear output, as the design's older three.js did — hex colours reach the screen as-is (deeper, more contrast). Light levels (`OFFICE3D_HEMI_INTENSITY`/`OFFICE3D_SUN_INTENSITY`) are tuned for that mode.
- **Walls** (`buildWalls`): thin 1.7 m slabs on the room side of each wall tile, a post where runs meet, a window every third tile of a straight run. The exit (`pickDoor`) prefers a framed door in a wall (camera-facing, then a side wall near the front) over an open floor edge.
...
- **Editing** (`editor3d.ts` + `components/BuildPanel.tsx`): the design's Build panel (right side, dock button "Build") — Select / Add floor (with floor finishes) / Walls / Remove, a furniture palette of to-scale footprint cards by category, the selected item's Turn / Remove, team rooms (draw one by dragging on the floor — `createRectRoom` — or add a ready-made `ROOM_TEMPLATES` room at the first free spot), land (grow 4 m per side, `handleGrowLayout`) and ready-made offices. It drives the same `editorState` + `useEditorActions` handlers as the canvas (tile under the pointer = first office surface hit, else the ground plane, so painting past the edge grows the map). Shift/middle-drag turns, right-drag erases (paint tools) or pans. Rugs (carpet colours), folder areas (add, paint, tie folders) and pets are in the panel too, so the classic editor panel never shows in 3D. Folder areas show as tinted floor while building; pets walk the office as small 3D animals (`buildPet`/`posePet`). The camera re-frames when the office's size or centre changes a lot (a new layout), not on small edits.
- **Camera + your avatar** (`avatar.ts`, `CameraPad` bottom-right): left-drag turns, right-drag / trackpad two-finger scroll moves, wheel / pinch zooms; keys WASD/arrows move, Q/E turn, +/- zoom, F frames the office (ignored while typing). The pad's buttons act as held keys (`keysRef`). "You" is the viewer's own character — client-only, never an agent, look in localStorage (`OFFICE3D_AVATAR_KEY`, edited in `LookModal`). Walk mode: the keys steer it (slides along walls, room glass only through the door via `canStep`), a floor click paths there, a click on an agent walks up to them (and opens the chat as usual); the camera follows.
- **Meeting panel** (top-right "Meeting" button): the meeting on now (who is at the table, who is talking, time left, End) or start one with chosen free agents (`MeetingDirector.startWith`). Meetings are a picture only — nothing is sent to agents.
- **Soft Dollhouse office** (`presets/softOffice.json` = `public/assets/default-layout-2.json`): kitchen, two desk pods, a glass team room with TV and table, a lounge. New offices get it (the server takes the highest `default-layout-N`); it keeps `layoutRevision: 1`, so existing layouts are NOT reset — Settings → Use Soft Dollhouse Office applies it as an undoable edit.
- **Loading the studio**: `lookStudioLazy.ts`, never `React.lazy` — the office's per-frame overlay updates can starve a Suspense retry forever (stuck on "Loading…"). Panels that hold the studio (`AddAgentModal`, `LookModal`) are right-side panels (`Modal side`).
