...
- **Ask an agent** (viewer header, and the pick bar): `AskAgentPanel.tsx` beside the open document — a question, who gets it (agents the office can type into, ranked by `rankAgents`: in the file's folder via the desk's `root`, idle first; or `startAgent` in the file's folder with the question as the first message), and refs to the picked places or the whole file (`askMessage`). Sent with `sendChatMessage` without leaving the viewer; the reply lands in the agent's chat (Open chat). The older "add to the open chat" route stays as the secondary button.
