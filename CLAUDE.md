...
- **Desktop app** (Electron, `adapters/electron/`) — a thin shell: `main.ts` spawns `dist/cli.js --no-open` with the user's `node` (falls back to Electron's own via `ELECTRON_RUN_AS_NODE`; node-pty may then not load), reads the tokened URL from the CLI's `Pixel Agents server running at` line, and loads it in a `BrowserWindow`. It never imports `server/`. Quit sends `SIGTERM` so the CLI's normal shutdown runs. `npm run electron` for dev; `electron-builder.yml` packages (`asar: false` + `npmRebuild: false`, because system Node runs the CLI). `updater.ts` = electron-updater against GitHub Releases, user-driven (packaged builds only; checks once per app launch and from the menu's Check for Updates… — no interval; nothing downloads or installs without a click — Download and install / Later / Skip this version, then Restart and install / Install when I quit / Not now; skipped version kept in userData `update-prefs.json`). No CI builds it: `npm run desktop:build` (installers in `release/`) or `GH_TOKEN=… npm run desktop:release` uploads to the GitHub release `v<package.json version>`, so bump the version to the tag first. macOS installs updates only when the app is signed (`CSC_LINK` / `CSC_KEY_PASSWORD` in the environment when building).
