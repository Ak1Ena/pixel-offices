...
- **Desktop app** (Electron, `adapters/electron/`) — a thin shell: `main.ts` spawns `dist/cli.js --no-open` with the user's `node` (falls back to Electron's own via `ELECTRON_RUN_AS_NODE`; node-pty may then not load), reads the tokened URL from the CLI's `Pixel Agents server running at` line, and loads it in a `BrowserWindow`. It never imports `server/`. Quit sends `SIGTERM` so the CLI's normal shutdown runs. `npm run electron` for dev; `electron-builder.yml` packages (`asar: false` + `npmRebuild: false`, because system Node runs the CLI). `updater.ts` = electron-updater against GitHub Releases (packaged builds only; startup + every 4 h; downloads in the background, offers a restart, installs on quit). `.github/workflows/publish-desktop.yml` builds all three OSes on a published release. macOS installs updates only when the app is signed (`MAC_CSC_LINK` secrets).
